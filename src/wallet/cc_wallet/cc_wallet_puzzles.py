...
def cc_make_solution(
    ...
    if auditees is not None:
        for auditee in auditees:
            aggees = f"{aggees}(0x{auditee[0]} 0x{auditee[1]} {auditee[2]} {auditee[3]})"
...
def create_spend_for_ephemeral(parent_of_e, auditor_coin, spend_amount):
    ...
    return CoinSolution(coin, clvm.to_sexp_f([puzzle, solution]))
...
def get_output_discrepancy_for_puzzle_and_solution(coin, puzzle, solution):
    return coin.amount - get_output_amount_for_puzzle_and_solution(
        puzzle, solution
    )
    ...
    while conditions != b"":
        ...
        if opcode == b"3":  # Check if CREATE_COIN
            ...
            if amount_str == "()":
                ...
                continue
            elif amount_str[:2] == "0x":  # Check for wonky decompilation
...
def check_is_cc_puzzle(puzzle: Program):
    ...
    if any(c not in string.hexdigits for c in inner_puzzle):
        return False
    genesisCoin = get_genesis_from_puzzle(puzzle_string)
    if any(c not in string.hexdigits for c in genesisCoin):
        return False
