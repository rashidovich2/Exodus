...
def main():
    ...
    if args.sk_seed is None and args.index is not None:
        log(
            'You have specified the -i (index) argument without the -s (sk_seed) argument. The program has changes, so that the sk_seed is now generated randomly, so -i is no longer necessary. Please run the program without -i.'
        )
        quit()
